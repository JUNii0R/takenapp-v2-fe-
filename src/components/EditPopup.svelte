...
import { updateTask } from "@src/helpers/tasks.js"
import { reloadTasklist } from "@src/helpers/Tasklist.js";

let taskKey = taskData.get().keyID;
let taskTitle = taskData.get().title;
let taskDesc = taskData.get().description;
let taskStatus = taskData.get().status;
...
<div class="container" on:click={() => {
  ...
  <form class="form-horizontal" on:submit|preventDefault={async() => {
        await updateTask({ taskKey, taskTitle, taskDesc, taskStatus });
        await reloadTasklist();
        closePopup();
      }}>
      <fieldset>
      
      <legend>Edit Task | <span>{taskData.get().title}</span></legend>
      ...
      <div class="form-group">
        ...
        <div class="col-xs-2">
        <input bind:value={taskDesc} id="Description" name="Description" type="text" placeholder="..." class="form-control input-md">
